...
export function connectAllSockets() {
    roomSocket = createWebSocket('/room/game', handleRoomMessage);
    chatSocket = createWebSocket('/chat/game', handleChatMessage);
    eventSocket = createWebSocket('/signal/event', handleEventMessage);
}
...
function handleEventMessage(msg) {
    ui.displayMessage(msg);
    if (['enter', 'leave', 'EVENT'].includes(msg.type)) {
