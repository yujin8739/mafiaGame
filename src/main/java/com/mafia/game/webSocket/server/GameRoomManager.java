...
//	    	    } else 
	...
	/* ---------- 전적 저장을 위한 메서드 추가 by 이수한 ---------- */
	public void saveGameResult(int roomNo, String winner) {
		Map<String, Object> result = gameRoomService.getRoomJob(roomNo); //해당 방의 USERLIST, JOB, STARTJOB 가져오기
	    String userListJson = clobToString((Clob) result.get("USERLIST"));
	    String jobJson = (String) result.get("JOB");
	    ...
	    try {
	        if (userListJson == null || jobJson == null) return;

	        List<String> userList = mapper.readValue(userListJson, new TypeReference<List<String>>() {});
	        List<Integer> jobList = mapper.readValue(jobJson, new TypeReference<List<Integer>>() {});

	        List<Job> jobDetails = gameRoomService.getJobDetails(jobList);  // jobNo에 해당하는 Job 객체들

	        for (int i = 0; i < userList.size(); i++) {
	            String userName = userList.get(i);
	            int jobNo = jobList.get(i);
	            Job job = jobDetails.get(i);

	            String team;
	            switch (job.getJobClass()) {
	                case 1: team = "마피아"; break;
	                case 2: case 4: team = "시민"; break;
	                case 3: team = "중립"; break;
	                default: team = "알 수 없음";
	            }

	            boolean isWin = (winner.equals("MAFIA_WIN") && team.equals("마피아")) ||
	                            (winner.equals("CITIZEN_WIN") && team.equals("시민")) ||
	                            (winner.equals("NEUTRALITY_WIN") && team.equals("중립"));

	            GameResult gameResult = new GameResult();
	            gameResult.setUserName(userName);
	            gameResult.setJobNo(jobNo);
	            gameResult.setTeam(team);
	            gameResult.setTeamResult(isWin ? "승리" : "패배");
	            //gameResult.setGameDate(new Date());

	            //gameRoomService.insertGameResult(gameResult); // Service → DAO에서 insert 수행
	        }
