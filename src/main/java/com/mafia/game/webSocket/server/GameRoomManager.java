...
public class GameRoomManager {
    ...
    @Value("${game.room.empty-delete-delay-ms:300000}")
    ...
    public void removeSession(int roomNo, WebSocketSession session, String userName) {
        ...
        if (gameSessions != null) {
            ...
            if (gameSessions.isEmpty()) roomGameSessions.remove(roomNo);
        }
    ...
    public void unbindChatSession(int roomNo, String userName) {
        ...
        if (chatSessions != null) {
            ...
            if (chatSessions.isEmpty()) roomChatSessions.remove(roomNo);
        }
    }
    ...
    public void updateStart(int roomNo) {
        ...
        try {
            ...
            if (jobCounts == null || jobCounts.isEmpty()) {
                if (totalPlayers == 6) { mafiaCount = 2; citizenCount = 4; }
                else if (totalPlayers == 7) { mafiaCount = 2; citizenCount = 5; }
                else if (totalPlayers == 8) { mafiaCount = 2; citizenCount = 5; neutralCount = 1; }
                else if (totalPlayers == 9) { mafiaCount = 2; citizenCount = 6; spyCount = 1; }
                else if (totalPlayers == 10) { mafiaCount = 3; citizenCount = 6; neutralCount = 1; spyCount = 1; }
                else if (totalPlayers == 11) { mafiaCount = 3; citizenCount = 7; neutralCount = 1; spyCount = 1; }
                else if (totalPlayers == 12) { mafiaCount = 3; citizenCount = 8; neutralCount = 1; }
                else if (totalPlayers == 13) { mafiaCount = 3; citizenCount = 8; neutralCount = 1; spyCount = 1; }
                else if (totalPlayers == 14) { mafiaCount = 3; citizenCount = 8; neutralCount = 2; spyCount = 1; }
                else if (totalPlayers == 15) { mafiaCount = 3; citizenCount = 9; neutralCount = 2; spyCount = 1; }
                else { throw new IllegalArgumentException("게임은 6명에서 15명까지만 가능합니다."); }
            } else {
            ...
            for (Job j : randomJobs) { jobArr.add(j.getJobNo()); }
            for (int i = 0; i < spyCount; i++) { jobArr.add(13); }
    ...
    public String checkWinner(int roomNo, int phaseIndex) {
        ...
        if (currentJobs == null || currentJobs.isEmpty()) return "CONTINUE";
        
        // 훼방꾼 승리 조건
        for (Map.Entry<String, Job> entry : userStartJobs.get(roomNo).entrySet()) {
            if (entry.getValue().getJobNo() == 7) { // 훼방꾼 찾기
                String saboteurUser = entry.getKey();
                Job saboteurCurrentJob = getJobForUser(roomNo, saboteurUser);
                if (saboteurCurrentJob != null && (saboteurCurrentJob.getJobNo() == 0 || saboteurCurrentJob.getJobName().contains("Ghost"))) {
                    if (phaseIndex <= 1) return "NEUTRALITY_WIN";
                }
                break; 
            }
        }
        ...
        for (Job job : currentJobs.values()) {
            if (job.getJobNo() != 0 && !job.getJobName().contains("Ghost")) {
                ...
                if (jobClass == 1 || jobClass == 5) aliveMafia++;
                else if (jobClass == 2 || jobClass == 4) aliveCitizen++;
                else if (jobClass == 3 || jobClass == 6) aliveNeutral++;
            }
        }

        if (aliveMafia >= aliveCitizen + aliveNeutral) return "MAFIA_WIN";
        if (aliveMafia == 0 && aliveNeutral == 0 && aliveCitizen > 0) return "CITIZEN_WIN";
        if (aliveMafia == 0 && aliveCitizen == 0 && aliveNeutral > 0) return "NEUTRALITY_WIN";
    ...
    public void updateJob(int roomNo, String userName, int newJobNo) {
        ...
        if (newJob != null) {
            userJobs.get(roomNo).put(userName, newJob);
             // TODO: DB에 변경된 직업 목록(JSON)을 업데이트하는 로직 필요
        }
    }
    ...
    public void updateJobToGhost(int roomNo, String userName) {
        ...
        if (startJob != null) {
            int startJobClass = startJob.getJobClass();
             if (startJobClass == 1 || startJobClass == 5) ghostJobNo = 1000;
             else if (startJobClass == 3 || startJobClass == 6) ghostJobNo = 2000;
        }
