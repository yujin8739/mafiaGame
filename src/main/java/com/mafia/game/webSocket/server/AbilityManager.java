...
public class AbilityManager {
    ...
    @Autowired
    public AbilityManager(@Lazy GameRoomManager roomManager, GameEventManager eventManager, GameRoomService gameRoomService, RoomHintService roomHintService, MemberService memberService) {
    ...
    public void useAbility(int roomNo, String userName, String targetUserName) {
        ...
        switch (jobName) {
            ...
            case "police":
                ...
                notifyAbilityResult(roomNo, userName, String.format("[%s]님은 %s.", targetUserName, isMafia ? "마피아입니다" : "마피아가 아닙니다"));
                break;
            case "robber":
                Job deadPersonJob = roomManager.getJobForUser(roomNo, targetUserName);
                if (deadPersonJob != null && (deadPersonJob.getJobNo() == 0 || deadPersonJob.getJobName().contains("Ghost"))) {
            ...
            case "hacker":
                 ...
                 for (Map.Entry<String, Job> entry : allUserJobs.entrySet()) {
                     ...
                     if (newHint != null) {
                         ...
                         RoomHint roomHint = new RoomHint(
                             roomNo,
                             currentUserName, // 힌트의 주체
                             newHint.getHint(),
                             currentUserNick
                         );
    ...
    public void processNightActions(int roomNo) {
        ...
        // 1. 마피아 킬 대상 결정
        int mafiaCount = (int) roomManager.getUserJobs(roomNo).values().stream().filter(j -> j.getJobClass() == 1).count();
        int maxKills = Math.max(1, mafiaCount / 2); // 마피아가 1명이어도 1명 킬 가능
        
        actions.getKillVotes().entrySet().stream()
            .sorted(Map.Entry.<String, Integer>comparingByValue().reversed())
            .limit(maxKills)
            .forEach(entry -> killedByMafia.add(entry.getKey()));

        // 2. 킬/힐/특수직 상호작용 처리
        for (String target : killedByMafia) {
            Job targetJob = roomManager.getJobForUser(roomNo, target);
            if (targetJob == null) continue;
            if (healedByDoctor.contains(target)) continue;
            ...
            if (targetJob.getJobNo() == 5 || targetJob.getJobNo() == 6) {
                ...
                Optional<String> partner = roomManager.getUserJobs(roomNo).entrySet().stream()
                    .filter(e -> e.getValue().getJobNo() == partnerJobNo)
    ...
    public void processVote(int roomNo) {
        NightlyAction actions = nightlyActions.get(roomNo);
        if (actions == null || actions.getVoteCounts().isEmpty()) return;

        String mostVotedUser = actions.getVoteCounts().entrySet().stream()
    ...
    private static class NightlyAction {
        ...
        public void addMafiaKill(String mafia, String target) { killVotes.merge(target, 1, Integer::sum); }
        public Map<String, Integer> getKillVotes() { return killVotes; }

        public void setDoctorHeal(String doctor, String target) { doctorHeals.put(doctor, target); }
        public Set<String> getHealTargets() { return new HashSet<>(doctorHeals.values()); }

        public void setPoliceCheck(String police, String target) { policeChecks.put(police, target); }

        public void addSpyCheck(String spy, String target) { spyChecks.computeIfAbsent(spy, k-> new CopyOnWriteArrayList<>()).add(target); }
        public Map<String, List<String>> getSpyChecks() { return spyChecks; }

        public void setNecromancerRevive(String necro, String target) { necromancerRevives.put(necro, target); }
        public String getRevivedTarget() { return necromancerRevives.values().stream().findFirst().orElse(null); }
        public String getReviver() { return necromancerRevives.keySet().stream().findFirst().orElse(null); }
        
        public void setRobberTarget(String robber, String target) { robberTargets.put(robber, target); }
        public String getRobbedTarget() { return robberTargets.values().stream().findFirst().orElse(null); }
        public String getRobber() { return robberTargets.keySet().stream().findFirst().orElse(null); }

        public void addVote(String voter, String target) { votes.put(voter, target); }
